...
- Added `GradientDumperCallback` for saving gradient shards during FSDP/HSDP training and `reconstruct_gradients.py` script for combining shards into full gradients. Includes DTensor-aware reconstruction that automatically detects shard dimensions and validates shapes.
